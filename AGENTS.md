...
JHEP-style paper, across many Codex sessions. **Continuity lives in files, not in the
...
0. Run `git fetch --quiet && git status -sb` **for this repo itself** (not for
   ...
   asking — the `Stop` hook in `.Codex/settings.json` commits and pushes them at the end
