...
1. **Load ground truth first.** Read the target section(s) in full from
   `paper/main.tex`, plus the `RESULTS.md` entries and the `notes/` files the
...
4. **Fix surgically, then verify**: apply the edits, `latexmk -cd -pdf projects/<slug>/paper/main.tex` must give
...
6. **Record any standing decision in the same pass.** Where a finding was resolved by
   ...
   settled with the user — write it into the `DECISIONS.md` before
