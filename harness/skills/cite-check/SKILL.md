...
Input: optional scope — a section, a list of bib keys, or "all" (default: every citation
in `paper/main.tex` and its included sources whose ledger row is missing, marked STALE,
or reported UNREVIEWED/STALE by REVIEW.json). Custom citation macros require manual inventory.
...
6. **Parallelize by paper, not by claim.** One agent per cited paper (or small group of
   ...
   use available primary-source retrieval tools; if alphaXiv is unavailable, use
   arXiv full text and INSPIRE. The orchestrating session merges the reports.

7. **Write the ledger `paper/CITATIONS.md`** (create from the template below if absent):
   one row per (claim, key), with the claim identified by its stable source ID plus first ~6 words verbatim
   (the same anchor convention as `PUNCHLINES.md`) plus the key, and the columns
   *type · source version · location · excerpt · support · originality · bib · date*. Rows survive edits
...
8. **Fix, then verify.** Every SECONDARY or NOT FOUND row is a finding: replace or add
   ...
   paragraph, PUNCHLINES untouched unless a claim changed. `latexmk -cd -pdf projects/<slug>/paper/main.tex` must give 0
...
9. **Record and commit.** The corrected attributions go into the `DECISIONS.md` (supersede in place), the ledger is committed with the draft, and the
...
Use `templates/CITATIONS.md`; do not maintain a second schema in this skill.
Include the stable source ID, claim anchor, key, source version, location, short
excerpt, support, originality, bib status and date. A partial-key review leaves the
block UNREVIEWED/STALE until all its citation claims have been checked.
